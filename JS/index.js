...
function plotOptTargetTrajectory() {
  ...
  s_max_display.innerHTML = `${
    Math.round(distTravelled(u, optTheta, g, h) * 100) / 100
  }m`;
  s_display.innerHTML = `${
    Math.round(distTravelled(u, theta, g, h) * 100) / 100
  }m`;
  ...
  angles.forEach((theta, index) => {
    ...
    if (index >= 3) {
      maxT =
        ((3 * u) / (2 * g)) *
        (Math.sin(theta) -
          Math.sqrt(Math.sin(theta) * Math.sin(theta) - 8 / 9));
      maxY = Math.sqrt(
        u * u * maxT * maxT -
          g * maxT * maxT * maxT * u * Math.sin(theta) +
          0.25 * g * g * maxT * maxT * maxT * maxT
      );
    }
    ...
    if (index >= 4) {
      ...
      minY = Math.sqrt(
        u * u * minT * minT -
          g * minT * minT * minT * u * Math.sin(theta) +
          0.25 * g * g * minT * minT * minT * minT
      );
    }

    const maxRangeTrace = {
      x: [
        projectilePosition(u, theta, g, h, maxT).x,
        projectilePosition(u, theta, g, h, minT).x,
      ],
      y: [
        projectilePosition(u, theta, g, h, maxT).y,
        projectilePosition(u, theta, g, h, minT).y,
      ],
      mode: "markers",
      name: "",
      marker: { color: "purple" },
...
function distTravelled(u, theta, g, h) {
  ...
  function z_func(z) {
    return (
      0.5 * Math.log(Math.abs(Math.sqrt(1 + z * z) + z)) +
      0.5 * z * Math.sqrt(1 + z * z)
    );
  }
  ...
  const R =
    ((u * u) / g) *
    (sinTheta * cosTheta +
      cosTheta * Math.sqrt(sinTheta * sinTheta + (2 * g * h) / (u * u)));
